...
function SkeletonCard() {
  return (
      <div className="animate-pulse rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
        <div className="h-40 rounded-2xl bg-slate-100 mb-4" />
...
function EmptyPortfolio() {
  return (
      <div className="col-span-full flex flex-col items-center rounded-3xl bg-white py-16 text-center shadow-sm ring-1 ring-slate-200">
        <div className="h-16 w-16 mx-auto rounded-2xl overflow-hidden ring-1 ring-slate-200">
          <img src="/logo-soyuz.jpeg" alt="Soyuz" className="h-full w-full object-cover" />
        </div>
        <p className="mt-5 text-lg font-semibold text-slate-900">Proyectos en camino</p>
...
/* ── Thumbnail de media para tarjetas de proyecto ─────────────────────────────── */
function ProjectThumbnail({ media, name }) {
  const items = Array.isArray(media) ? media : [];
  const video = items.find((m) => m.media_type === "video");
  const image = items.find((m) => m.media_type === "image");
  ...
  if (image) {
    return <img src={mediaUrl(image.storage_key)} alt={name} className="h-full w-full object-cover" />;
  }

  return (
      <div className="h-full w-full flex items-center justify-center">
        <img src="/logo-soyuz.jpeg" alt={name} className="h-16 w-16 rounded-2xl object-cover ring-2 ring-white/20 opacity-80" />
      </div>
  );
}
...
export default function Portfolio() {
  ...
  async function toggleFeatured(id, cur) {
    ...
    try {
      ...
      if (r.ok) {
        ...
        // Re-fetch public portfolio to keep in sync
        const pr = await fetch("/api/public/portfolio");
        if (pr.ok) {
          const d = await pr.json();
          setProjects(d?.data || []);
  ...
  async function saveProjectInfo(projectId) {
    ...
    setSaving(true);
    try {
      const impArr = (draft.implementations || "")
          .split(",")
          .map((s) => s.trim())
          .filter(Boolean);
  ...
  async function handleMediaUpload(projectId) {
    ...
    try {
      ...
      if (r.ok) {
        ...
        setMediaCaption("");
        input.value = "";
        // Re-fetch portfolio to update thumbnails
        const pub = await fetch("/api/public/portfolio");
        if (pub.ok) {
          const d = await pub.json();
          setProjects(d?.data || []);
        ...
        setUploadError(errData?.message || `Error ${r.status} al subir archivo.`);
      }
    } catch (err) {
  ...
  // Proyecto destacado = el primero con is_featured, o el primero de la lista
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-14 sm:py-20">
                    ...
                    <div key={p.id} className="rounded-xl bg-white/5 ring-1 ring-white/10 overflow-hidden">
                      <div className="flex items-center justify-between px-4 py-3">
                        ...
                        <div className="flex items-center gap-2 shrink-0 flex-wrap">
                          <button
                              onClick={() => toggleVis(p.id, p.is_public)}
                                ...
                                if (editingProjectId === p.id) {
                                  ...
                                  setEditDrafts((prev) => ({
                                    ...
                                    [p.id]: {
                                      ...
                                      portfolioDescription: p.portfolio_description || "",
                                      implementations: (p.implementations || []).join(", "),
                                    },
                                  }));
                                }
                              }}
                      ...
                      {editingProjectId === p.id && (
                          <div className="border-t border-white/10 bg-white/5 p-4 space-y-4">
                            {/* Editar info */}
                            <div className="space-y-2">
                              <p className="text-[10px] font-bold uppercase text-cyan-400 tracking-wide">
                                Información del proyecto
                              </p>
                              <input
                                  type="text"
                                  value={editDrafts[p.id]?.name ?? ""}
                                  onChange={(e) =>
                                      setEditDrafts((prev) => ({
                                        ...prev,
                                  ...
                                  placeholder="Título"
                                  className="w-full rounded-lg bg-slate-800 px-3 py-2 text-sm text-white ring-1 ring-white/10"
                              />
                              <textarea
                                  rows={2}
                                  value={editDrafts[p.id]?.description ?? ""}
                                  onChange={(e) =>
                                      setEditDrafts((prev) => ({
                                        ...prev,
                              ...
                              <input
                                  type="text"
                                  value={editDrafts[p.id]?.implementations ?? ""}
                                  onChange={(e) =>
                                      setEditDrafts((prev) => ({
                                        ...prev,
                                        [p.id]: { ...(prev[p.id] || {}), implementations: e.target.value },
                                      }))
                                  }
                                  placeholder="Implementaciones (separar con comas: React, Node.js, PostgreSQL)"
                                  className="w-full rounded-lg bg-slate-800 px-3 py-2 text-xs text-slate-300 ring-1 ring-white/10"
                              />
                              <button
                                  onClick={() => saveProjectInfo(p.id)}
                                  disabled={saving}
                            ...
                            {/* Media */}
                            <div>
                              <p className="text-[10px] font-bold uppercase text-cyan-400 tracking-wide mb-2">
                                Fotos y videos ({projectMedia.length})
                              </p>

                              {/* Preview de archivos multimedia existentes */}
                              {projectMedia.length > 0 && (
                                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-3">
                                    {projectMedia.map((m) => (
                                        <div
                                            key={m.id}
                                            className="relative rounded-lg overflow-hidden ring-1 ring-white/20 group"
                                        >
                                          {m.media_type === "image" ? (
                                              <img
                                                  src={mediaUrl(m.storage_key)}
                                                  alt={m.caption || m.original_name}
                                                  className="h-24 w-full object-cover bg-slate-800"
                                                  onError={(e) => {
                                          ...
                                          <div
                                              className="h-24 items-center justify-center bg-slate-800 text-xs text-slate-400"
                                              style={{ display: "none" }}
                                          >
                                            {m.media_type === "image" ? "🖼" : "🎬"} {m.original_name}
                                          </div>
                                          <p className="px-2 py-1 text-[10px] text-slate-300 truncate">
                                            {m.media_type === "video" ? "🎬 " : "🖼 "}
                                            {m.caption || m.original_name}
                                          </p>
                                          <button
                              ...
                              {/* Upload error */}
                              {uploadError && (
                                  <div className="mb-2 rounded-lg bg-red-500/20 px-3 py-2 text-xs text-red-300 ring-1 ring-red-500/30">
                                    {uploadError}
                                  </div>
                              )}
                              ...
                              <div className="flex flex-wrap items-end gap-2">
                                ...
                                <button
                                    onClick={() => handleMediaUpload(p.id)}
                                    disabled={uploading}
        ...
        <div className="mt-10 grid gap-8 lg:grid-cols-3">
          ...
          <div className="rounded-3xl bg-white shadow-sm ring-1 ring-slate-200 overflow-hidden">
            <div className="aspect-video bg-gradient-to-br from-slate-900 to-slate-800 overflow-hidden">
              <video className="h-full w-full object-cover" controls preload="metadata" playsInline poster="/logo-soyuz.jpeg">
        ...
        {status === "error" && (
            <div className="mt-10 rounded-3xl bg-white p-8 text-center shadow-sm ring-1 ring-slate-200">
              ...
              <Button as={Link} to="/contacto" variant="primary" className="mt-6">
                                    ...
                                    <div className="mt-4">
                                      ...
                                      <div className="flex flex-wrap gap-1.5">
                                        {featured.implementations.map((imp) => (
                                            <span
                                                key={imp}
                                                className="rounded-full bg-cyan-50 px-2.5 py-0.5 text-[10px] font-medium text-cyan-700 ring-1 ring-cyan-200"
                                            >
                                  {imp}
                                </span>
                                        ))}
                          ...
                          <div className="lg:col-span-5 grid gap-6">
                            ...
                            >
                              <div className="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-200">
                                <p className="text-sm font-semibold text-slate-900">Portal exclusivo para clientes</p>
                        ...
                        <div className="mt-10">
                          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                            {rest.map((project) => (
                                <div
                                    key={project.id}
                                    className="rounded-3xl bg-white shadow-sm ring-1 ring-slate-200 hover:shadow-md transition overflow-hidden"
                                >
                                  {/* Thumbnail de media */}
                                  <div className="h-44 bg-gradient-to-br from-slate-900 to-slate-800 overflow-hidden">
                                    <ProjectThumbnail media={project.media} name={project.name} />
                                  </div>
                                ...
                                {serviceIcon(project.service_type)}
                                      ...
                                      <div>
                                        ...
                                        <p className="mt-1 text-sm text-slate-600">
                                          {project.description || "Proyecto entregado con éxito por Soyuz."}
                                        </p>
                                      </div>
                                    </div>
                            ...
                            <span className="rounded-full bg-slate-50 px-3 py-1 text-xs font-semibold text-slate-700 ring-1 ring-slate-200">
                              ...
                              {project.status === "deployed" ? "Desplegado" : "Entregado"}
                            </span>
                                    </div>
                                    ...
                                    {project.implementations && project.implementations.length > 0 && (
                                        <div className="mt-3 flex flex-wrap gap-1">
                                          {project.implementations.map((imp) => (
                                              <span
                                                  key={imp}
                                                  className="rounded-full bg-cyan-50 px-2 py-0.5 text-[9px] font-medium text-cyan-700 ring-1 ring-cyan-200"
                                              >
                                  {imp}
                                </span>
                                          ))}
                                    ...
                                    <div className="mt-5">
                                      <Button variant="secondary" className="w-full" as={Link} to="/contacto">
                                        Cotizar similar
                                      </Button>
                                    </div>
                                  </div>
