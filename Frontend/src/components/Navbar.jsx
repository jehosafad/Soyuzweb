              ...
              <div id="soyuz-mobile-menu" className="pb-4 md:hidden">
                <div className="mt-2 rounded-2xl bg-white p-2 shadow-sm ring-1 ring-slate-200/70">
                  <div className="grid gap-1">
                    {links.map((l) => (
                        <NavLink key={l.label} to={l.to} className={navLinkClass} end onClick={() => setOpen(false)}>
                          {l.label}
                        </NavLink>
                    ))}
                    ...
                    {session ? (
                        <>
                          ...
                          <Button as={Link} to={portalPath} variant="primary" className="mt-1 w-full" onClick={() => setOpen(false)}>
                            Mi Portal
                          </Button>
                        </>
                    ) : (
                        <>
                          <Link to="/login" className="rounded-xl px-3 py-2 text-sm font-medium text-slate-600 transition hover:bg-slate-50 hover:text-slate-900" onClick={() => setOpen(false)}>
                            Iniciar sesión
                          </Link>
                          <Button as={Link} to="/contacto" variant="primary" className="mt-1 w-full" onClick={() => setOpen(false)}>
                            Cotizar proyecto
                          </Button>
                        </>
                    )}
